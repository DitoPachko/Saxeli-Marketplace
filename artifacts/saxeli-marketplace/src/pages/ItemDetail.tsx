    ...
    <div className="mx-auto max-w-[1280px] px-5 py-7 md:px-10 md:py-10">
      ...
      <div className="mt-7 grid gap-9 lg:grid-cols-[minmax(0,1.06fr)_minmax(360px,.94fr)] lg:gap-14">
        ...
        <section className="enter">
          ...
          <div className="mt-10 border-t border-[hsl(var(--border))] pt-6"><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-[hsl(var(--muted-foreground))]">გამყიდველი</p><div className="mt-4 flex items-center gap-3"><Avatar initials={item.seller.initials} size="md" testId="img-seller-avatar" /><div><p className="font-semibold" data-testid="text-seller-name">{item.seller.name}</p><p className="mt-1 flex items-center gap-2 text-xs text-[hsl(var(--muted-foreground))]"><Star size={13} className="fill-[hsl(var(--primary))] text-[hsl(var(--primary))]" /> {item.seller.rating.toFixed(1)} · {item.seller.listings} განცხადება</p></div><span className="ml-auto rounded-lg bg-[hsl(var(--accent)/.14)] px-2 py-1 text-[10px] text-[hsl(var(--foreground))]">{item.seller.responseTime ?? 'სწრაფი პასუხი'}</span></div></div>
